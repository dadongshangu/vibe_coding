# 教程索引

> Vibe Coding 和 Cursor 完整教程体系

## 📚 教程导航

### 🎓 Vibe Coding 教程

#### 基础教程
- **[初学者完全指南](vibe-coding/beginner-guide.md)** ⭐ 推荐从这里开始
  - 适合零基础学习者
  - 从概念到实践
  - 包含常见问题解答

#### 进阶教程
- **[高级指南](vibe-coding/advanced-guide.md)**
  - Prompt 工程高级技巧
  - 代码质量保证
  - 安全性深入
  - 性能优化
  - 大型项目管理

- **[Prompt 工程完全指南](vibe-coding/prompt-engineering.md)** ⭐ 提高效率必备
  - Prompt 设计原则
  - 上下文管理技巧
  - 不同场景的模板
  - 高级技巧

#### 实战教程
- **[实战项目案例](vibe-coding/case-studies.md)**
  - 完整项目开发流程
  - 真实项目案例
  - 经验总结

- **[故障排除指南](vibe-coding/troubleshooting.md)**
  - 常见问题解决
  - 调试技巧
  - 性能问题排查

### 🛠️ Cursor 教程

#### 基础参考
- **[快速参考手册](cursor/quick-reference.md)**
  - 快捷键速查
  - 常用命令
  - 配置建议

#### 高级功能
- **[高级功能指南](cursor/advanced-features.md)**
  - Composer 深度使用
  - Multi-Agents 应用
  - 代码库索引
  - 性能调优

## 🗺️ 学习路径

### 路径 1：快速入门（1周）
1. [初学者完全指南](vibe-coding/beginner-guide.md)
2. [快速参考手册](cursor/quick-reference.md)
3. 完成第一个项目

### 路径 2：系统学习（1个月）
1. [初学者完全指南](vibe-coding/beginner-guide.md)
2. [Prompt 工程完全指南](vibe-coding/prompt-engineering.md)
3. [高级指南](vibe-coding/advanced-guide.md)
4. [Cursor 高级功能指南](cursor/advanced-features.md)
5. [实战项目案例](vibe-coding/case-studies.md)

### 路径 3：深入学习（2-3个月）
1. 完成路径 2 的所有内容
2. 独立完成多个项目
3. 深入研究 [故障排除指南](vibe-coding/troubleshooting.md)
4. 分享经验和贡献

## 📖 教程特点

- ✅ **从零开始**：适合完全初学者
- ✅ **循序渐进**：从基础到高级
- ✅ **实战为主**：每个概念都有实例
- ✅ **问题解决**：专门的故障排除指南
- ✅ **持续更新**：基于最新信息

## 🔗 相关资源

- [项目主页](/vibe_coding/)
- [实践项目](../examples/)
- [参考资源](vibe-coding/beginner-guide.md#参考资源)

---

---

<div style="display: flex; justify-content: space-between; align-items: center; margin: 40px 0; padding: 20px; background: #f5f5f5; border-radius: 8px; flex-wrap: wrap;">
  <div style="flex: 1; min-width: 150px;">
    <p style="margin: 0; color: #666; font-size: 14px;">上一篇</p>
    <p style="margin: 5px 0 0 0; color: #999;">
      已是第一篇
    </p>
  </div>
  <div style="flex: 1; text-align: center; min-width: 150px;">
    <p style="margin: 0;">
      <a href="/vibe_coding/" style="color: #0366d6; text-decoration: none; font-weight: 500;">🏠 返回主页</a>
    </p>
  </div>
  <div style="flex: 1; text-align: right; min-width: 150px;">
    <p style="margin: 0; color: #666; font-size: 14px;">下一篇</p>
    <p style="margin: 5px 0 0 0;">
      <a href="/vibe_coding/tutorials/vibe-coding/beginner-guide.html" style="color: #0366d6; text-decoration: none; font-weight: 500;">Vibe Coding 初学者完全指南 →</a>
    </p>
  </div>
</div>

---

**选择适合你的路径，开始学习吧！** 🚀
