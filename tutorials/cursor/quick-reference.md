...
# 带任务启动
agent "重构 auth 模块为 JWT"

# 脚本/无头模式（只输出不改文件）
agent -p "分析这段代码"

# 脚本中允许改文件
agent -p --force "添加 JSDoc 注释"
```

完整安装步骤、模式说明与故障排除见 **[Cursor CLI 安装与使用](cursor-cli.md)**。
...
<div style="display: flex; justify-content: space-between; align-items: center; margin: 40px 0; padding: 20px; background: #f5f5f5; border-radius: 8px; flex-wrap: wrap;">
  ...
  <div style="flex: 1; text-align: right; min-width: 150px;">
    ...
    <p style="margin: 5px 0 0 0;">
      <a href="../claude-code/beginner-guide.md" style="color: #0366d6; text-decoration: none; font-weight: 500;">Claude Code 入门指南 →</a>
    </p>
  </div>
</div>
